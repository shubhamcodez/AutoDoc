import { z } from 'zod';
import { terminalSize, WebSocket, generateRandomBase64 } from '@lmstudio/lms-isomorphic';
...
const chatMessagePartSubPartFunctionCallRequestDataSchema = z.object({
    arguments: z.record(jsonSerializableSchema).optional(),
    name: z.string(),
});
const chatMessagePartSubPartToolCallRequestSchema = z.object({
...
const downloadedModelSchema = z.discriminatedUnion("type", [
    z.object({
...
const llmLlamaAccelerationSettingSchema = z.object({
...
const llmChatHistoryMessageContentPartSchema = z.discriminatedUnion("type", [
    z.object({
        type: z.literal("text"),
        text: z.string(),
    }),
...
const llmChatHistoryMessageContentSchema = z.array(llmChatHistoryMessageContentPartSchema);
const llmChatHistoryRoleSchema = z
    .string()
    .refine(v => ["system", "user", "assistant"].includes(v), {
    message: "Invalid role, must be one of 'system', 'user', or 'assistant'",
});
const llmChatHistoryMessageSchema = z.object({
    role: llmChatHistoryRoleSchema,
    content: llmChatHistoryMessageContentSchema,
});
const llmChatHistorySchema = z.array(llmChatHistoryMessageSchema);
z.object({
    history: llmChatHistorySchema,
});
const llmConversationContextInputSchema = z.array(z.object({
    role: llmChatHistoryRoleSchema,
    content: z.string(),
}));
const llmCompletionContextInputSchema = z.string();
...
const llmJinjaInputFormatSchema = z.enum([
    "promptOnly",
    "promptWithImages",
...
const llmStructuredPredictionSettingSchema = z.discriminatedUnion("type", [
    z.object({
        type: z.literal("none"),
    }),
    z.object({
...
const llmPredictionConfigSchema = z.object({
...
class ModelNamespace {
    ...
    async get(param) {
        const stack = getCurrentStack(1);
        this.validator.validateMethodParamOrThrow(`client.${this.namespace}`, "get", "param", z.union([reasonableKeyStringSchema, modelQuerySchema]), param, stack);
        let query;
...
class EmbeddingDynamicHandle extends DynamicHandle {
    ...
    async embedString(inputString) {
        const stack = getCurrentStack(1);
        inputString = this.validator.validateMethodParamOrThrow("client.embedding", "embedString", "inputString", z.string(), inputString, stack);
    ...
    async tokenize(inputString) {
        const stack = getCurrentStack(1);
        inputString = this.validator.validateMethodParamOrThrow("model", "tokenize", "inputString", z.string(), inputString, stack);
...
export { ChatHistory, ChatMessage, LMStudioClient, MaybeMutable, createConfigSchematics, kvValueTypesLibrary };
